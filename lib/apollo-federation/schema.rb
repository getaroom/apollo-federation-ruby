...
module ApolloFederation
  module Schema
    ...
    module CommonMethods
      ...
      def federation(version: '1.0', default_link_namespace: nil, linkd: [], compose_directives: [])
      ...
      def federation_2_prefix
        ...
        schema += formatted_links
        schema += formatted_compose_directives
      ...
      def formatted_links
        all_links.map do |link|
          directives = ["url: \"#{link[:url]}\""]
          directives << "as: \"#{link[:as]}\"" if link[:as]

          if link[:import]
            imports = link[:import].map { |directive| "\"@#{directive}\"" }.join(', ')
            directives << "import: [#{imports}]"
          end

          "  @link(#{directives.join(', ')})"
        end
      end

      def formatted_compose_directives
        compose_directives.map do |directive|
          "  @composeDirective(name: \"@#{directive}\")"
        end
      end
